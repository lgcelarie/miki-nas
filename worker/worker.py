import pika, sys, os, subprocess
import functools
import logging
import requests
from pika.exchange_type import ExchangeType

LOG_FORMAT = ('%(levelname) -10s %(asctime)s %(name) -30s %(funcName) '
              '-35s %(lineno) -5d: %(message)s')
LOGGER = logging.getLogger(__name__)

logging.basicConfig(level=logging.INFO, format=LOG_FORMAT)

def on_message(chan, method_frame, header_frame, body, userdata=None):
    src_folder='/origin'
    dst_folder='/destination'
    """Called when a message is received. Log message and ack it."""
    LOGGER.info('Delivery properties: %s, message metadata: %s', method_frame, header_frame)
    LOGGER.info('Userdata: %s, message body: %s', userdata, body)
    if body.decode() == 'sync':
            print('Recieved sync message. Processing...')
            subprocess.run(f'mv {src_folder} {dst_folder}/', shell=True)
            print('Moving process completed.')
            print('Sending plex the request to sync library')
            params = {'X-Plex-Token': os.getenv('PLEX_TOKEN')}
            try:
                response = requests.get('192.168.1.152:32400/library/sections/29/refresh', params=params)
                print('Request to plex server sent... mission complete!')
            except requests.exceptions.ConnectionError:
                print('A connection error ocurred')
            except requests.exceptions.HTTPError:
                print(f'a {response.status_code} was returned')
    else:
        print('unknown message received')
    chan.basic_ack(delivery_tag=method_frame.delivery_tag)

def main():
    """Main method."""
    credentials = pika.PlainCredentials('guest', 'guest')
    parameters = pika.ConnectionParameters('rabbitmq', credentials=credentials)
    connection = pika.BlockingConnection(parameters)

    channel = connection.channel()
    channel.exchange_declare(
        exchange='plex',
        exchange_type=ExchangeType.direct,
        passive=False,
        auto_delete=False)
    channel.queue_declare(queue='plex')
    channel.queue_bind(
        queue='plex', exchange='plex', routing_key='plex')
    channel.basic_qos(prefetch_count=1)

    on_message_callback = functools.partial(
        on_message, userdata='on_message_userdata')
    channel.basic_consume('plex', on_message_callback)

    try:
        channel.start_consuming()
    except KeyboardInterrupt:
        channel.stop_consuming()

    connection.close()


if __name__ == '__main__':
    main()
